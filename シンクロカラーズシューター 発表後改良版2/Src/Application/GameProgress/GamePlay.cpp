...
void C_GamePlay::Update(bool p_diff)
{
	...
	else
	{
		...
		if(!p_diff)
		{
			...
			if (m_timer->GettotalTime() <= 60)
			{
				EnemyApp(1, 499, 0, m_enemyList);
				EnemyApp(1, 999,3, m_multi_enemyList);

			}
			else
			{
				EnemyApp(2, 499, 0, m_enemyList);
			}
	...
	std::vector<std::shared_ptr<C_EnemyBase>>& p_list)
{
	...
	//==============================
	// 全滅したColor_codeの敵を集める
	//==============================
	std::vector<std::shared_ptr<C_EnemyBase>> respawnCandidates;
	...
	for (auto& group : colorGroups)
	{
		...
		if (!exist)
		{
			for (auto& e : enemies)
			{
				if (!e->GetMflg())
				{
					respawnCandidates.push_back(e);
				}
			}
		}
	}
	//==============================
	// 候補から全体で p_RespawnNum 体出す
	//==============================
	if (!respawnCandidates.empty())
	{
		std::shuffle(
			respawnCandidates.begin(),
			respawnCandidates.end(),
			gen);

		int spawnCount =
			std::min(p_RespawnNum,
				(int)respawnCandidates.size());

		for (int i = 0; i < spawnCount; i++)
		{
			respawnCandidates[i]->App(p_list);
		}
	}
