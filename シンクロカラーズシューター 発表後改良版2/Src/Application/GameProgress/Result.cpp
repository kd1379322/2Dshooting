...
void C_Result::Update()
{
	...
	m_scaleMat = Math::Matrix::CreateScale(0.5f, 0.5f, 0);
	m_rotationMat = Math::Matrix::CreateRotationZ(-0.1f);
	m_transMat = Math::Matrix::CreateTranslation(310, 160, 0);
	m_b_mat = m_scaleMat * m_rotationMat * m_transMat;
...
void C_Result::Draw2D()
{
	...
	if (f_life == 1)
	{
		rect = {350,1830,590,80};
	}
	else if(f_life == 2)
	{
		rect = {350,1710,590,80 };
	}
	else if (f_life == 3)
	{
		rect = { 350,1590,590,80 };
	}
...
void C_Result::SetScore(int s, int life)
{
	f_life = life;
	m_score = s + (life*10000);
}
