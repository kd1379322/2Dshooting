...
void C_Ranking::Update()
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 10; j++)
		{
			...
			m_transMat = Math::Matrix::CreateTranslation(128 * j * 0.45f - 250, 127 * i - 90, 0);
...
void C_Ranking::Draw()
{
	...
	for (int i = 0; i < 10; i++)
	{
		NumDraw(Rk[0], 10, i, m_Nummat[0][i]);
	}

	for (int i = 0; i < 10; i++)
	{
		NumDraw(Rk[1], 10, i, m_Nummat[1][i]);
	}

	for (int i = 0; i < 10; i++)
	{
		NumDraw(Rk[2], 10, i, m_Nummat[2][i]);
	}
}
